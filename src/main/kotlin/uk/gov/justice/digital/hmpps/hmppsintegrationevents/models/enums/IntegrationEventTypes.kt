...
enum class IntegrationEventTypes(val value: String, val registrationTypes: List<String>?, val upstreamEventTypes: List<String>) {
  ...
  companion object {
    fun from(value: String, registrationType: String?): IntegrationEventTypes? =
      IntegrationEventTypes.entries.firstOrNull { when (registrationType) {
        null -> it.upstreamEventTypes.contains(value)
        else -> it.upstreamEventTypes.contains(value) && it.registrationTypes!!.contains(registrationType)
      } }
  }
}
