...
@ActiveProfiles("test")
@JsonTest
class HmppsDomainEventsListenerSANReviewUpdatedTest {
  ...
  fun `will process a san review schedule updated notification`() {
    ...
    verify(exactly = 1) { hmppsDomainEventService.execute(hmppsDomainEvent, listOf(IntegrationEventType.SAN_REVIEW_SCHEDULE_CHANGED)) }
  }
}
