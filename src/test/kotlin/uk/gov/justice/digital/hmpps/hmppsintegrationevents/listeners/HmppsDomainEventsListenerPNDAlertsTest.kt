...
class HmppsDomainEventsListenerPNDAlertsTest : HmppsDomainEventsListenerTestCase() {
  ...
  fun `will process and save a pnd alert for person alert created event`(alertCode: String) = onDomainEventShouldCreateEventNotification(
  ...
  @ParameterizedTest
  @ArgumentsSource(AlertCodeArgumentSource::class)
  fun `will process and save a pnd alert for person alert changed event`(alertCode: String) = onDomainEventShouldCreateEventNotification(
  ...
  @ParameterizedTest
  @ArgumentsSource(AlertCodeArgumentSource::class)
  fun `will process and save a pnd alert for person alert deleted event`(alertCode: String) = onDomainEventShouldCreateEventNotification(
  ...
  @ParameterizedTest
  @ArgumentsSource(AlertCodeArgumentSource::class)
  fun `will process and save a pnd alert for person alert updated event`(alertCode: String) = onDomainEventShouldCreateEventNotification(
    eventType = "person.alert.updated",
    alertCode = alertCode,
    description = "An alert has been updated in the alerts service",
  )

  private fun onDomainEventShouldCreateEventNotification(
    eventType: String,
    alertCode: String,
